...
module Form = 
    ...
    module Orm = 
        ...
        let inline tableName< ^T > ( this : OrmState ) = 
            match context< ^T > this with 
            | TcsContext -> 
                let attr = typedefof< ^T >.GetCustomAttribute(typeof< TcsTableAttribute >, false) 
                if isNull attr then
                    typedefof< ^T >.Name
                else 
                    ( attr :?> TcsTableAttribute ).Value
            | ReportingContext 
            | _ -> 
                let attr = typedefof< ^T >.GetCustomAttribute(typeof< ReportingTableAttribute >, false) 
                if isNull attr then
                    typedefof< ^T >.Name
                else 
                    ( attr :?> ReportingTableAttribute ).Value
        ...
        let inline columnMapping< ^T > ( this : OrmState ) = 
            ...
            |> Array.mapi ( fun i x -> 
                let sqlName = 
                    ...
                    | TcsContext -> 
                        let attr = 
                            x.GetCustomAttribute(typeof< TcsColumnAttribute >, false) 
                        if isNull attr then
                            x.Name
                        else 
                            ( attr :?> TcsColumnAttribute ).Value
                            
                    | ReportingContext 
                    | _ -> 
                        let attr = 
                            x.GetCustomAttribute(typeof< ReportingColumnAttribute >, false) 
                        if isNull attr then
                            x.Name
                        else 
                            ( attr :?> ReportingColumnAttribute ).Value
        ...
        let inline selectLimit< ^T > lim  ( this : OrmState ) = 
            ...
            use reader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
            try 
                Ok ( generateReader< ^T > reader this )
            with 
        ...
        let inline insert< ^T > ( binder : IDbCommand -> IDbCommand )  ( this : OrmState ) =
            ...
            binder cmd 
            |> fun x -> x.ExecuteNonQuery()
    
    
    let tcs = MSSQL( Config.Get "tcs", TcsContext)
    let reporting = MySQL( Config.Get "reporting", ReportingContext)
    let vital = MySQL( Config.Get "", VitalContext)